    ...
    if dev_ui_enabled:
        ...
        if st.session_state.get("dev_unlocked", False):
            try:
                ...
                st.markdown(
                    "<div class='small'>Counts are stored in external PostgreSQL. "
                    "Visits are counted once per session, not exact unique people.</div>",
                    unsafe_allow_html=True
                )
            except Exception as e:
...
""",
            ...
            core_df = res[["_rid","Chemical compound","Cas","Smiles","Logk","k","Chemical_class_27","Ph","T","Ref"]].rename(
...
<div class="vbox"><div class="vtitle">k</div><div class="vvalue">{kval}</div></div>
    ...
    if res is None or (isinstance(res, pd.DataFrame) and len(res) == 0):
        ...
        with tabs[1]:
            ...
            if mpack is not None:
                ...
                out_df = pd.DataFrame([[rid, system, name, cas, smiles, logk_raw, k_raw] + bits],
                                      columns=["Record_ID","System","Chemical compound","Cas","Smiles","Logk","k"] + cols)
